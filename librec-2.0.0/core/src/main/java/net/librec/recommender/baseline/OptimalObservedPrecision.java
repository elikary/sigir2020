 ...
 */
public class OptimalObservedPrecision extends AbstractRecommender {
    ...
    protected double predict(int userIdx, int itemIdx) throws LibrecException {
        if (!itemScores.containsKey(itemIdx)) {
            ...
            long relTrainUsers = DoubleStream.of(itemRatingsVector.getData()).filter(rating -> rating > 0).count();
            long trainUsers = itemRatingsVector.getCount();
            double score = relTrainUsers * 1.0/ (numUsers - trainUsers);
