 ...
 * @edited by Elisa Mena-Maldonado
 */
    ...
    private void readData(String dataColumnFormat, String inputDataPath, double binThold) throws IOException {
        ...
        for (File dataFile : files) {
            ...
            while ((len = fileRead.read(buffer)) != -1) {
                ...
                // edited by @Elisa Mena-Maldonado
                // included copy array to solve first empty element in array that caused IndexOutOfBounds exception
                if (bufferData[0].equals(""))
                {
                    bufferData = Arrays.copyOfRange(bufferData, 1, bufferData.length);
                }
                //
